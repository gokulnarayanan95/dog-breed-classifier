...
def train_model(images, labels, global_step):
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            binary_inputs = classifier_model.inputs(False)
            sess.close()
        ...
        for i in range(len(binary_inputs)):
            ...
            with tf.device('/cpu:0'):
                image_bin = np.load(binary_inputs[i][0])
                label_bin = np.load(binary_inputs[i][1])
                images = tf.convert_to_tensor(image_bin, tf.float32, "Images")
                labels = tf.convert_to_tensor(label_bin, tf.int64, "labels")

            train_model(images,
                        tf.reshape(labels, [classifier_model.BATCH_SIZE]),
                        global_step)
